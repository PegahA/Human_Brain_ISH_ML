...
def disease_embed_evaluate(study):
    ...
    #ts_list = ["1602225390"]
    ts_list = ["1602861872", "1602226166", "1602225390"]
...
def evaluate(ts, not_found_list):
    ...
    if not os.path.exists(path_to_embeddings):
        ...
        for item in contents:
            if "test" in item:
                if item.endswith("embeddings_image_level_visual.csv") and 'autism' not in item and 'schizophrenia' not in item:
                   image_level_files_list.append(item)
            ...
            if "validation" in item:
                if item.endswith(
                    ...
                    image_level_files_list.append(item)
                if item.endswith(
                        "embeddings_image_level_temporal.csv") and 'autism' not in item and 'schizophrenia' not in item:
                    image_level_files_list.append(item)
